...
def main():
    ...
    labeled_num = len(placeholder_token_ids) - len(args.unlabelled)
    # Resize the token embeddings as we are adding new special tokens to the tokenizer
    text_encoder.resize_token_embeddings(len(tokenizer))

    # Initialise the newly added placeholder token with the embeddings of the initializer token
    token_embeds = text_encoder.get_input_embeddings().weight.data
    # token_embeds.requires_grad_(False)
    with torch.no_grad():
        for token_id in placeholder_token_ids:
            token_embeds[token_id] = token_embeds[initializer_token_id].clone()
    # add_token_embeds = token_embeds[placeholder_token_ids]
    ...
    head = DINOHead(in_dim=768, out_dim=args.out_dim, nlayers=3)
    ...
    para +=[{'params': text_encoder.get_input_embeddings().parameters(), 'weight_decay': args.adam_weight_decay}]
    ...
    train_dataset = TextualInversionDataset(
        ...
        transform = trans,
        placeholder_token=tokenizer.convert_ids_to_tokens(placeholder_token_ids),
    ...
    text_encoder, model, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        text_encoder, model, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder,model):
                ...
                # shape = batch["input_ids"].size()
                aug_image = torch.cat([batch["image"],batch["strong_aug_image"]],dim=0).to(accelerator.device)
                student_proj,student_out = model(aug_image)
                ...
                #     label_out = torch.cat([x[label_index] for x in student_out.chunk(2)],dim=0)
                #     label_loss = F.cross_entropy(label_out / 0.1,\
                #                     batch["target"][label_index].repeat(2).to(accelerator.device),
                #                     reduction="mean")
                #     cluster_loss += label_loss
                ...
                #     student_proj = torch.cat([f[label_index].unsqueeze(1) for f in student_proj.chunk(2)], dim=1)
                #     student_proj = torch.nn.functional.normalize(student_proj, dim=-1)
                #     sup_con_labels = batch["target"][label_index]
                #     sup_con_loss = SupConLoss()(student_proj, labels=sup_con_labels)
                #     cluster_loss += sup_con_loss
                # unsupervised, unsup
                # unlabel_index = torch.where(~batch["is_labelled"])[0]
                # unlabel_index = [i for i in torch.arange(0,len(batch["is_labelled"]))]
                # if epoch>20 and len(unlabel_index) > 0:
                #     unlabel_out = torch.cat([x[unlabel_index] for x in student_out.chunk(2)],dim=0)
                #     pseudo_label = torch.argmax(unlabel_out[:,labeled_num:], dim=1)
                #     pseudo_label = pseudo_label + labeled_num
                #     pseudo_label_loss = F.cross_entropy(unlabel_out / 0.1,\
                #                                         pseudo_label, reduction="mean")
                #     cluster_loss += pseudo_label_loss
                cluster_loss *=0.1
                weight = (student_out.chunk(2)[0] / 0.1).softmax(dim=-1)
                encoder_hidden_states = text_encoder(weight,batch["input_ids"])[0].to(dtype=weight_dtype)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    torch.save(save_dict, './model/{}_{}.pt'.format(args.output_dir.split('/')[-1],
                                                                    global_step))
            ...
            logs = {"loss": loss.detach().item(),"cluster_loss": cluster_loss.detach().item(),
    ...
    torch.save(save_dict, './model/{}.pt'.format(args.output_dir.split('/')[-1]))
